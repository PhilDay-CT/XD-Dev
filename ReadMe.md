...
`docker-compose-sisl/` contains the files to run a "SISL" cross domain system using the released images from [IN423-xd](https://github.com/ConfiguredThings/IN423-xd).  These files are defined as extensions to the standard files in `docker-compose/`, showing that in general only the image has to be changed to create a SISL system.
...
```
cd docker-compose-sisl
docker-compose -f dev/mono-local.yml up 
```

By editing the compose files in `docker-compose-sisl/` various "SISL gateway" issues can be simulated:
